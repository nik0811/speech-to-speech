...
def get_tts_handler(module_kwargs, stop_event, lm_response_queue, send_audio_chunks_queue, should_listen, parler_tts_handler_kwargs, melo_tts_handler_kwargs, chat_tts_handler_kwargs, facebook_mms_tts_handler_kwargs):
    if module_kwargs.tts == "parler":
        ...
        # Disable torch compilation completely for ParlerTTS
        import torch._dynamo
        torch._dynamo.config.disable = True
        
        # Also disable torch compile logs
        import torch._logging
        torch._logging.set_logs(dynamo=False)
        
        # Monkey patch StaticCache to handle deprecated max_batch_size
        from parler_tts.modeling_parler_tts import StaticCache
        ...
        # Re-enable torch compilation for other models
